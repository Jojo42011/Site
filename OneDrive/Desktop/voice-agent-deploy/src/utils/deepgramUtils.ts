...
import { pushPcm16Chunk } from "./audioCapture";

// Retrieve Deepgram API key from the backend so /demo works in production where
// build-time env injection is unavailable. This endpoint should be protected
// behind your deployment controls; it's meant only for the hosted demo.
export const getApiKey = async (): Promise<string> => {
  const resp = await fetch("/api/deepgram-key");
  if (!resp.ok) {
    throw new Error("Failed to load Deepgram API key");
  }
  const json = await resp.json().catch(() => ({}));
  const apiKey = json?.key;
  if (!apiKey) {
    throw new Error("Deepgram API key not found on server");
  }
  return apiKey;
};
...
export const sendMicToSocket = (socket: WebSocket) => (event: AudioProcessingEvent) => {
  ...
  // Keep a rolling buffer for Audio Intelligence analysis.
  pushPcm16Chunk(audioDataToSend.buffer);
